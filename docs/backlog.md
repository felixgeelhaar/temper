...
## Continue server.go decomposition (spec, patch, authoring, analytics, docindex, pairing handlers)

FOLLOWUP. The first decomposition pass (commit pending) extracted sandbox + tracks handlers into handlers_sandbox.go + handlers_tracks.go (-425 lines from server.go). Continue extracting: handlers_spec.go (handleCreateSpec, handleListSpecs, handleGetSpec, handleValidateSpec, handleMarkCriterionSatisfied, handleLockSpec, handleGetSpecProgress, handleGetSpecDrift, handleGenerateSpec); handlers_patch.go (handlePatchPreview, handlePatchApply, handlePatchReject, handleListPatches, handlePatchLog, handlePatchStats); handlers_authoring.go (handleAuthoringDiscover, handleAuthoringSuggest, handleAuthoringApply, handleAuthoringHint); handlers_analytics.go (handleGetProfile, handleAnalyticsOverview, convertToDomainProfile, handleAnalyticsSkills, handleAnalyticsErrors, handleAnalyticsTrend); handlers_docindex.go (handleDocIndex* x5); handlers_pairing.go (handlePairing, handlePairingWithEscalation, handlePairingStream, handleHint, handleReview, handleStuck, handleNext, handleExplain, handleEscalate); handlers_exercises.go + handlers_session.go + handlers_run.go for the rest. Goal: server.go retains only NewServer, Start, Shutdown, setupRoutes, middleware wiring, json helpers, health/ready/status/config, buildLevelModelMap. Target size &lt; 600 lines.

---

## Editor surfaces for --why rationale (VS Code, Neovim, MCP, CLI)

FOLLOWUP. The pairing service now produces a rich Intervention.Rationale ("Selected L2 for intent=stuck; hint dependency 25%; topic=go/concurrency (skill 0.42); difficulty=advanced; build errors present; policy clamp=L3; model=claude-haiku-4-5") on every intervention. Surface it in editor clients: (1) VS Code extension — show in command output panel under a collapsible "Why this hint?" section, accepting Cmd+Shift+H,W to toggle; (2) Neovim Lua plugin — render in floating window beneath the hint with `:TemperWhy` toggle; (3) MCP server — return rationale as a separate tool result block when the caller passes show_why=true; (4) CLI — once temper hint/review/etc commands exist, add --why flag that prints rationale alongside the intervention content. Acceptance: each client renders the rationale verbatim from the intervention payload (the daemon already includes Rationale in JSON responses).
